...
function countUp(session) {
    fetch(`${api_url}?session=${session}&action=up`)
...
function countDown(session) {
    fetch(`${api_url}?session=${session}&action=down`)
...
function getCount(session) {
    fetch(`${api_url}?session=${session}&action=get`)
        .then(res => res.json())
...
function buildSession(session) {
    fetch(`${api_url}?session=${session}&action=connect`)
        .then("Session built.")
        .catch(`call failed for ${session}`)
}
...
function setup(session) {
    ...
    if(display != null) {
        ...
        setInterval(() => getCount(session), 1000)
    }
